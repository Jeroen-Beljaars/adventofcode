...
in the list above, because bus ID 19 must depart seven minutes after the timestamp at which bus ID 7 departs,
...
def get_timestamp(dts):
    step = 1
    ts = 100000000000000
    dts = [(int(x), ix) for ix, x in enumerate(dts.split(",")) if x != 'x']
...
if __name__ == '__main__':
    ...
    print(f"The answer is: {get_timestamp(input_data)}")
