...
discover that no ships embark from that port to your vacation island.
...
def select_best_bus(input_data):
    timestamp, busses = input_data.split("\n")
    timestamp = int(timestamp)
    bus_offsets = {
        int(bus): int(bus) - (timestamp % int(bus))
        for bus in set(busses.split(",")) if bus.isnumeric()
    }

    sorted_busses = sorted(bus_offsets.items(), key=lambda x: x[1])

    return sorted_busses[0][0] * sorted_busses[0][1]


if __name__ == '__main__':
    input_data = open("input.txt", "r").read()

    print(f"The answer is: {select_best_bus(input_data)}")
