...
on a high-speed train. However, the train ticket you were given is in a language you don't understand.
...
303,401,402,403; of course, the actual train tickets you're looking at are much more complicated.
...
def generate_rules(rules_string):
    rules = {}
    rule_pattern = r"(^.*): ([0-9]*)-([0-9]*) or ([0-9]*)-([0-9]*)$"
    for index, rule in enumerate(rules_string.split("\n")):
        name, *rule_range = re.findall(rule_pattern, rule)[0]
        rules[name] = rule_range

    return rules


def validate_tickets(input_data):
    data_split = input_data.split("\n\n")

    rules = generate_rules(data_split[0])

    nearby_tickets = data_split[2].replace("nearby tickets:\n", "").split("\n")
    invalid_values = 0
    valid_tickets = []
...
if __name__ == '__main__':
    ...
    invalid_values, _ = validate_tickets(input_data)
    print(f"The answer is: {invalid_values}")
