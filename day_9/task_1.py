...
two of the 25 immediately previous numbers. The two numbers will have different values, and there might be more than one such pair.
...
def validate_numbers(numbers, lookback=DEFAULT_LOOKBACK):
    """
    Check if a number is the sum of any of the 2 previous 25 nums

    :param numbers:     List of numbers to use
    :param lookback:    Amount of numbers to look back
    :return:            The product of the two numers that sum to the desired sum.
    """
    for index, number in enumerate(numbers[lookback:], lookback):
        valid = False
        previous_numbers = numbers[index - lookback: index]

        for prev_index, prev_number in enumerate(previous_numbers):
            required_num = number - prev_number

            if required_num in previous_numbers and previous_numbers.index(required_num) != prev_index:
                valid = True
                break
