...
"""
DEFAULT_STARTING_JOLT = 0

def calculate_jolt_difference(jolts, starting_jolt=DEFAULT_STARTING_JOLT):
    differences = {
        1: 0,
        2: 0,
        3: 0
    ...
    built_in_joltage = max(jolts) + 3
    jolts.append(built_in_joltage)

    jolt = starting_jolt
    while jolt < built_in_joltage:
        for difference in range(1, 4):
            new_jolt = jolt + difference
            if jolt + difference in jolts:
...
if __name__ == '__main__':
    ...
    print(f"The invalid number is {calculate_jolt_difference(input_data)}")