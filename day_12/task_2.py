...
it produced extremely circuitous instructions. When the captain uses the PA system to ask if anyone
...
At the end of these instructions, the ship's Manhattan distance (sum of the absolute values of its east/west position and
its north/south position) from its starting position is 17 + 8 = 25.
...
NORTH, EAST, SOUTH, WEST, LEFT, RIGHT, FORWARD = "N", "E", "S", "W", "L", "R", "F"

degree_map = {0: EAST, 90: SOUTH, 180: WEST, 270: NORTH}

def shift_dict(target, shifts):
    target_len = len(target)
    target_keys = list(target.keys())

    return {k: target[target_keys[int((i + shifts) % target_len)]] for i, k in enumerate(target_keys)}
...
def handle_instructions(instructions):
    ...
    for instruction in instructions:
        direction, amount = re.findall(r"([NESWLRF])([0-9]*)", instruction)[0]
        amount = int(amount)

        if direction in [LEFT, RIGHT]:
            shifts = (current_degree + amount if direction == LEFT else current_degree - amount) / 90
            waypoint = shift_dict(waypoint, shifts)
        elif direction in [NORTH, EAST, SOUTH, WEST]:
            waypoint[direction] += amount
        else:
            for key, value in waypoint.items():
                ship[key] += amount * value

    return abs(ship[EAST] - ship[WEST]) + abs(ship[NORTH] - ship[SOUTH])

if __name__ == '__main__':
    input_data = open("input.txt", "r").read().split("\n")[:-1]

    print(f"The euclidean distance is {handle_instructions(input_data)}")
